...
rt <- search_tweets("#GERSWE AND lang:de", 
                    include_rts = F,
                    retryonratelimit = T,
                    n=200000)

save(rt, file = "../data/gerswe.Rda")
```
...
saveWidget(w,"positiv.html",selfcontained = F)
webshot::webshot("positiv.html","positiv.png",vwidth = 1000, vheight = 800, delay =10)
```
...
saveWidget(w,"negativ.html",selfcontained = F)
webshot::webshot("negativ.html","negativ.png",vwidth = 1000, vheight = 800, delay =10)
```
...
grouped <- ggplot(animate) +
  ...
  # Tor Schweden
  geom_vline(xintercept = gamestart + minutes(32), color = "yellow", linetype = 2) +
  # Tor DEU
  geom_vline(xintercept = gamestart + minutes(48) + minutes(3) + minutes(15), color = "black", linetype = 2) +
  # Tor DEU
  geom_vline(xintercept = gamestart + minutes(90) + minutes(8) + minutes(15), color = "black", linetype = 2) +
  ...
  labs(x=NULL, y=NULL, title = "Sentiment Wert von Tweets zu GER-SWE",
       subtitle = "Durchschnittlicher Wert der Tweets, die den Hashtag #GERSWE enhielten",
       caption = "Die Tweets wurden über die Twitter REST API mit Hilfe des R-Packetes rtweet gesammelt")

gganimate(grouped, interval=.3, "gerswe.gif", ani.width=800, ani.height=400)
```
